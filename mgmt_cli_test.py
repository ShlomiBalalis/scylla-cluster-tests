...
class MgmtCliTest(BackupFunctionsMixIn, ClusterTester):
    ...
    def test_enospc_during_backup(self):
        ...
        self.generate_load_and_wait_for_results()
        backup_task = mgr_cluster.create_backup_task(location_list=location_list)
        backup_task.wait_for_status(list_status=[TaskStatus.RUNNING], step=60)
        backup_task.stop()

        Nemesis.reach_enospc_on_node(target_node=target_node, log_object=self.log)
        ...
        assert backup_task.status != TaskStatus.DONE, "The backup failed to run on a node with no free space, while it" \
                                                      " should have had the room for snapshots due to the previous run"
